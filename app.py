...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App']).sum().loc[:,['Reviews','Rating']].sort_values(by = 'Reviews', ascending = False).head(10).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ## Bar Plot
